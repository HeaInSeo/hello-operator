...
//
// kube-slint의 SnapshotFetcher/PreFetch 지원 revision부터는
// 기본 curlpod fetcher가 Session.Start() 시점의 시작 스냅샷을 캐시한다.
// 이 테스트는 별도 snapshotFetcher 주입 없이 기본 fetcher 경로를 사용한다.
package e2e
...
func TestHelloSLIE2E(t *testing.T) {
	...
	// 4. SLI 세션: 기본 curlpod fetcher가 Start() 시점 스냅샷을 캡처한다.
	session := harness.NewSession(harness.SessionConfig{
		...
		GateOnLevel: "fail",
		CurlImage:   sliCurlImage,
	})

	// Start()는 workload 변경 전에 호출되어야 pre-change snapshot이 캐시된다.
	session.Start()
	...
	// 7. End()는 종료 시점 스냅샷을 수집하고, Start()에서 캐시한 시작 스냅샷과 비교한다.
