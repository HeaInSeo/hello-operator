...
### 8.2 개념 설명: 관리실 전화기와 외부 전용 회선

네트워크 바인딩과 방화벽을 이해하기 위해 아파트 관리실 전화기를 상상해보자.

**비유: 관리실 전화기를 외부 전용 회선으로 연결하는 작업**

아파트 관리실에는 두 종류의 전화기가 있다.

- **내선 전화기(127.0.0.1 = localhost)**: 건물 내부 사람끼리만 연결되는 전화기.
  외부에서는 이 번호로 전화를 걸 수 없다. Tilt가 기본으로 사용하는 바인딩 주소다.

- **외부 공개 회선(0.0.0.0 = 모든 인터페이스)**: 외부에서도 전화를 걸 수 있는 회선.
  `--host 0.0.0.0` 옵션은 내선 전화기에 외부 회선을 연결하는 작업이다.
  이 작업을 하지 않으면 랩탑(외부)에서 아무리 전화를 걸어도 연결되지 않는다.

- **교환원(방화벽 = firewalld)**: 어떤 외부 전화번호(포트)를 연결할지 결정한다.
  등록되지 않은 번호로 오는 전화는 교환원이 차단한다. `10350` 포트가 허용 목록에
  없으면 연결이 거부된다.

- **Tailscale VPN(사설 전화망)**: Tailscale로 연결된 장치끼리는 같은 건물 내선처럼
  통신한다. 개발 서버의 Tailscale 전화번호: `100.92.45.46`.

```
[랩탑 브라우저]
     |
     | Tailscale 전화 (100.92.45.46:10350)
     v
[교환원: firewalld public zone]  ← 10350 포트가 없으면 여기서 차단
     |
[관리실 전화기: Tilt 프로세스]   ← --host 127.0.0.1이면 외부 회선 미연결
```

두 개의 장벽이 존재한다:
1. Tilt가 내선(127.0.0.1)에만 연결되어 외부 전화를 받을 수 없다.
2. 교환원(방화벽)이 `10350` 포트 번호의 전화를 허용 목록에 올려두지 않았다.
